...
function Player.resetTrackedMissions(self, missionStorages)
	...
	for _, storage in pairs(missionStorages) do
		local mission = self:getMissionByStorage(storage)
		local quest = self:getQuestByMission(mission)
...
function Player.sendQuestLine(self, quest)
	...
	local msg = NetworkMessage()
	msg:addByte(0xF1)
	msg:addU16(0) --38f is actual value needed?
...
function Player.sendTrackedQuests(self, remainingQuests, missions)
	...
	msg:addByte(0xD0)
	msg:addByte(0x01)
	msg:addByte(remainingQuests) --38f
