...
void PlayerWheel::sendOpenWheelWindow(NetworkMessage &msg, uint32_t ownerId) {
	...
	m_player.client->sendResourceBalance(RESOURCE_BANK, 500 * 1000 * 1000 + m_player.getBankBalance());
	m_player.client->sendResourceBalance(RESOURCE_INVENTORY_MONEY, 500 * 1000 * 1000 + m_player.getMoney());
...
uint16_t PlayerWheel::getExtraPoints() const {
	...
	const std::string wheelPointsKey = "wheel-of-destiny-scroll-points-";
...
int32_t PlayerWheel::getGiftOfCooldown() const {
	const int32_t value = m_player.getStorageValueByKey(STORAGEVALUE_GIFT_OF_LIFE_COOLDOWN_WOD);
	if (value <= 0) {
		return 0;
	}
	return value;
