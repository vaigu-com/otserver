...
private:
	...
	// Vaigu custom
	void schedulePlayerQuickLoot(
		const std::shared_ptr<Player> &player,
		const Position &pos,
		uint16_t itemId,
		uint8_t stackPos,
		const std::shared_ptr<Item> &defaultItem,
		bool lootAllCorpses,
		bool autoLoot
	);
	bool isPlayerInLootRange(const std::shared_ptr<Player> &player, const Position &pos);
	bool walkPlayerToCorpse(
		const std::shared_ptr<Player> &player,
		const Position &pos,
		uint16_t itemId,
		uint8_t stackPos,
	...
	std::shared_ptr<Item> getItemToLoot(
		const std::shared_ptr<Player> &player,
		const Position &pos,
		uint8_t stackPos,
		uint16_t itemId,
	...
	std::shared_ptr<Container> getCorpseFromItem(
		const std::shared_ptr<Item> &item,
		const Position &pos
	);
	bool isCorpseLootable(
		const std::shared_ptr<Player> &player,
	...
	void handleDirectLoot(
		const std::shared_ptr<Player> &player,
		const std::shared_ptr<Item> &item,
		const std::shared_ptr<Container> &corpse
	);
	void handleCorpseLoot(
		const std::shared_ptr<Player> &player,
		const std::shared_ptr<Container> &corpse,
		const Position &pos,
		bool lootAll
	);
	void sendLootMessageWithCooldown(
		const std::shared_ptr<Player> &player,
