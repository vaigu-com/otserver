...
void IOPrey::parsePreyAction(std::shared_ptr<Player> player, PreySlot_t slotId, PreyAction_t action, PreyOption_t option, int8_t index, uint16_t raceId) const {
	...
	if (action == PreyAction_GridReroll) {
		...
		player->sendMessageDialog("Changing prey with this option checked will reduce bonus by 3 stars. Rerolling with gold reduces by 1 star. Selecting from list doesn't reduce stars.");
		...
		if (option == PreyOption_AutomaticReroll && player->getPreyCards() < static_cast<uint64_t>(g_configManager().getNumber(PREY_BONUS_REROLL_PRICE))) {
			...
			player->sendMessageDialog("You don't have enough prey cards to lock the monster and the bonus when the slot expire.");
			return;
		}
		...
		if (option == PreyOption_AutomaticReroll) {
			...
			player->sendMessageDialog("Changing prey with this option checked will reduce bonus by 2 stars. Rerolling with gold reduces by 1 star. Selecting from list doesn't reduce stars.");
		}
