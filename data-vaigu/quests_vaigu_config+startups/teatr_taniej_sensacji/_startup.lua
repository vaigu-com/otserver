...
local startupItemsStatic = {
	...
	{ pos = { 6184, 1019, 7 }, id = 1020, aid = Storage.TheaterOfCheapThrills.Spawns.JuerdoTitsgo },
	{
		...
		requiredState = { [Storage.TheaterOfCheapThrills.Questline] = 1 },
	},
}
...
local startupItemsLibrumVortex = {
	...
	{ pos = { -5, -1, -2 }, id = 6260, aid = Storage.TheaterOfCheapThrills.Mission04 },
}
...
local function resetSpawnLocks()
	for _, spawnLock in pairs(Storage.TheaterOfCheapThrills.Spawns) do
...
local function createPlusShapeTiles()
	...
	for _, config in pairs(TEATR_TANIEJ_SENSACJI_PLUS_SHAPE.aidToMachineConfig) do
		IterateBetweenPositions(config.topLeft, config.downRight, function(context)
			...
			item:setActionId(Storage.TheaterOfCheapThrills.PlusShapePuzzle.Tile)
		end)
	end
end
