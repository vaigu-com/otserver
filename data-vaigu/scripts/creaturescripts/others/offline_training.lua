...
function offlineTraining.onLogin(player)
	...
	if table.contains({ SKILL_CLUB, SKILL_SWORD, SKILL_AXE, SKILL_DISTANCE }, offlineTrainingSkill) then
		updateSkill = player:addOfflineTrainingTries(offlineTrainingSkill, ((trainingTime * 0.8) / 2) / (offlineTrainingSkill == SKILL_DISTANCE and 4 or 2))
	elseif offlineTrainingSkill == SKILL_MAGLEVEL then
		local gainTicks = (topVocation:getManaGainTicks() / 1000) * 2
		if gainTicks == 0 then
			gainTicks = 1
		end
		updateSkill = player:addOfflineTrainingTries(SKILL_MAGLEVEL, trainingTime * ((topVocation:getManaGainAmount() / gainTicks) * 1.5)) -- 340k mana/h
	end
