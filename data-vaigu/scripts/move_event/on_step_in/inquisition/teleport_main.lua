local teleports = {
	...
	[2157] = { text = "Entering the Blood Halls.", bossStorage = Storage.zuguroshTimer, newPos = Position(6978, 1709, 13), storage = 2 }, -- from zugurosh ward {x = 6978, y = 1709, z = 13}
	...
	[2162] = { text = "Entering the Vats.", bossStorage = Storage.madarethTimer, newPos = Position(6759, 1878, 11), storage = 3 }, -- from madareth ward {x = 6759, y = 1878, z = 11}
	...
	[2168] = { text = "Entering the Arcanum.", bossStorage = Storage.latrivanTimer, newPos = Position(6687, 1922, 12), storage = 4 }, -- from demon forge {x = 6687, y = 1922, z = 12}
	...
	[2174] = { text = "Entering the Hive.", bossStorage = Storage.annihilonTimer, newPos = Position(6806, 1782, 11), storage = 5 }, -- from annihilon ward {x = 6803, y = 1799, z = 10}
	...
	[2178] = { text = "Entering the Shadow Nexus. Abandon all Hope.", bossStorage = Storage.hellgorakTimer, newPos = Position(6716, 1778, 11), storage = 6 }, -- from hellgorak ward {x = 6716, y = 1778, z = 11}
...
function movement.onStepIn(creature, item, toPosition, fromPosition)
	...
	if teleport.alwaysSetStorage and player:getStorageValueByKey(Storage.TheInquisition.EnterTeleport) < teleport.storage then
		player:setStorageValueByKey(Storage.TheInquisition.EnterTeleport, teleport.storage)
	end

	if teleport.bossStorage then
		if item.uid == 2168 then
		end
		if Game.getStorageValueByKey(teleport.bossStorage) == 2 then
			if player:getStorageValueByKey(Storage.TheInquisition.EnterTeleport) < teleport.storage then
				if item.uid == 2168 then
				end
				player:setStorageValueByKey(Storage.TheInquisition.EnterTeleport, teleport.storage)
			end
	...
	elseif teleport.storage and player:getStorageValueByKey(Storage.TheInquisition.EnterTeleport) < teleport.storage then
