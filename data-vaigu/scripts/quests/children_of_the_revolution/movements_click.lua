...
function doClearMissionArea()
	Game.setStorageValueByKey(Storage.Quest.U8_54.ChildrenOfTheRevolution.Mission05, -1)
	...
	for i = 1, #spectators do
		...
		if spectator:isPlayer() then
			...
			if spectator:getStorageValueByKey(Storage.Quest.U8_54.ChildrenOfTheRevolution.Questline) == 19 then
				spectator:setStorageValueByKey(Storage.Quest.U8_54.ChildrenOfTheRevolution.Questline, 20)
			end
		else
			spectator:remove()
		end
...
function click.onStepIn(creature, item, position, fromPosition)
	...
	if player:getStorageValueByKey(Storage.Quest.U8_54.ChildrenOfTheRevolution.Questline) ~= 19 or Game.getStorageValueByKey(Storage.Quest.U8_54.ChildrenOfTheRevolution.Mission05) == 1 then
		return true
	end
