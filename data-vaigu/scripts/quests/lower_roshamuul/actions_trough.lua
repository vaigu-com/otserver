...
function lowerRoshamuulTrough.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target:getId() == 20216 then
		...
		player:setStorageValueByKey(Storage.Quest.U10_30.RoshamuulQuest.Roshamuul_Mortar_Thrown, math.max(0, player:getStorageValueByKey(Storage.Quest.U10_30.RoshamuulQuest.Roshamuul_Mortar_Thrown)) + 1)
	end
	return true
end
