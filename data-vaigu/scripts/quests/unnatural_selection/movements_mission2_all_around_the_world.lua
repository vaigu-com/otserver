...
function mission2AllAroundTheWorld.onStepIn(creature, item, position, fromPosition)
	...
	local targetValue = config[player:getStorageValueByKey(Storage.Quest.U8_54.UnnaturalSelection.Mission02)]
	...
	if table.contains(targetValue.position, player:getPosition()) and player:getItemCount(10159) >= 1 then
		...
		player:setStorageValueByKey(Storage.Quest.U8_54.UnnaturalSelection.Mission02, player:getStorageValueByKey(Storage.Quest.U8_54.UnnaturalSelection.Mission02) + 1)
