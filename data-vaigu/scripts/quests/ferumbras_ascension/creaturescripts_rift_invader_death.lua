...
function riftInvaderDeath.onDeath(creature, corpse, lasthitkiller, mostdamagekiller, lasthitunjustified, mostdamageunjustified)
	...
	for i = 1, #crystals do
		...
		if creature:getPosition():isInRange(crystal.fromPosition, crystal.toPosition) then
			if player:getStorageValueByKey(crystal.globalStorage) > 8 then
				local item = Tile(crystal.crystalPosition):getItemById(14955)
				if not item then
					return true
				end
				item:transform(14961)
				player:setStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.Crystals.AllCrystals, player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.Crystals.AllCrystals) + 1)
			end
			if player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.Crystals.AllCrystals) == 8 then
			...
			player:setStorageValueByKey(crystal.globalStorage, player:getStorageValueByKey(crystal.globalStorage) + 1)
