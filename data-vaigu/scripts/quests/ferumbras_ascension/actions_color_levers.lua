...
function ferumbrasAscendantColorLevers.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.FirstDoor) >= 1 then
		item:transform(item.itemid == 9125 and 9126 or 9125)
		return true
	end
	if item.actionid == 54381 then
		if player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.ColorLever) < 1 then
			local rand = math.random(4)
			player:setStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.ColorLever, rand)
	...
	elseif item.actionid == 54382 then
		if player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.ColorLever) == 1 then
			player:setStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.FirstDoor, 1)
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You select the correct colour.")
			toPosition:sendMagicEffect(CONST_ME_POFF)
		end
	elseif item.actionid == 54383 then
		if player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.ColorLever) == 3 then
			player:setStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.FirstDoor, 1)
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You select the correct colour.")
			toPosition:sendMagicEffect(CONST_ME_POFF)
		end
	elseif item.actionid == 54384 then
		if player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.ColorLever) == 4 then
			player:setStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.FirstDoor, 1)
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You select the correct colour.")
			toPosition:sendMagicEffect(CONST_ME_POFF)
		end
	elseif item.actionid == 54385 then
		if player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.ColorLever) == 2 then
			player:setStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.FirstDoor, 1)
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You select the correct colour.")
			toPosition:sendMagicEffect(CONST_ME_POFF)
		end
