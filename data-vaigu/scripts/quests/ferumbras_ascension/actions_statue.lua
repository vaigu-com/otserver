...
function ferumbrasAscendantStatue.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if Tile(Position(33415, 32379, 12)):getItemById(22163) or player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.Fount) < 4 or player:getStorageValueByKey(Storage.Quest.U10_90.FerumbrasAscension.Statue) >= 1 then
		return false
	end
	for i = 1, #fount do
		local fount = fount[i]
