...
function spiritMagical.onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if item:getId() == 4049 then
		if itemEx.itemid == 1979 then
			if player:getStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.Mission01 == 1) then
				local qStorage = player:getStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.TombUse)
				if qStorage < 3 then
					...
					player:setStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.TombUse, qStorage + 1)
	...
	elseif item:getId() == 4050 then
		if itemEx.itemid == 5993 then -- ghost
			...
			if player:getStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.CharmUse) < 1 then
				player:setStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.CharmUse, 1)
			else
				player:setStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.GhostUse, player:getStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.GhostUse) + 1)
			end
		...
		elseif itemEx.itemid == 11675 or itemEx.itemid == 11676 then -- souleater
			...
			player:setStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.SouleaterUse, player:getStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.SouleaterUse) + 1)
			itemEx:transform(itemEx:getType():getDecayId())
			toPosition:sendMagicEffect(CONST_ME_MAGIC_GREEN)
		elseif itemEx.itemid == 9001 or itemEx.itemid == 9002 then --nightstalker
			...
			player:setStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.NightstalkerUse, player:getStorageValueByKey(Storage.Quest.U8_7.SpiritHunters.NightstalkerUse) + 1)
			itemEx:transform(itemEx:getType():getDecayId())
			toPosition:sendMagicEffect(CONST_ME_MAGIC_GREEN)
		end
	end
