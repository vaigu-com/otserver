...
function beaverTrees.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if toPosition == config.treesBeaver[1] or toPosition == config.treesBeaver[2] or toPosition == config.treesBeaver[3] and player:getStorageValueByKey(TheNewFrontier.Questline) == 5 then
		if toPosition == config.treesBeaver[1] and player:getStorageValueByKey(TheNewFrontier.Mission02.Beaver1) < 1 then
			for i = 1, 3 do
				local position = toPosition
				Game.createMonster("enraged squirrel", position)
				toPosition:sendMagicEffect(CONST_ME_TELEPORT)
			end
			player:setStorageValueByKey(TheNewFrontier.Mission02.Beaver1, 1)
			player:say("You have marked the tree, but you also angered the aquirrel family who lived on it!", TALKTYPE_MONSTER_SAY)
		elseif toPosition == config.treesBeaver[2] and player:getStorageValueByKey(TheNewFrontier.Mission02.Beaver2) < 1 then
			for i = 1, 5 do
				local position = toPosition
			...
			toPosition:sendMagicEffect(CONST_ME_TELEPORT)
			player:setStorageValueByKey(TheNewFrontier.Mission02.Beaver2, 1)
		...
		elseif toPosition == config.treesBeaver[3] and player:getStorageValueByKey(TheNewFrontier.Mission02.Beaver3) < 1 then
			Game.createMonster("thieving squirrel", toPosition)
			toPosition:sendMagicEffect(CONST_ME_TELEPORT)
			player:setStorageValueByKey(TheNewFrontier.Mission02.Beaver3, 1)
