...
function iceCrack.onStepIn(creature, item, position, fromPosition)
	...
	if Game.getStorageValueByKey(GlobalStorage.IceCrack) ~= 1 then
		for a = 1, #config do
			Game.createItem(config[a].itemId, 1, config[a].position)
			addEvent(removeIceCrack, 5 * 60 * 1000, config[a].position, config[a].itemId)
		end
		Game.setStorageValueByKey(GlobalStorage.IceCrack, 1)
		addEvent(Game.setStorageValueByKey, 5 * 60 * 1000, GlobalStorage.IceCrack, 0)
	end
	return true
end
