...
function foreshockTransform.onThink(creature)
	...
	Game.setStorageValueByKey(GlobalStorage.HeartOfDestruction.ForeshockHealth, creature:getHealth())
	local foreshockStage = Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.ForeshockStage) > 0 and Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.ForeshockStage) or 0
	...
	for index, value in pairs(monsterTable) do
		if hpPercent <= index and foreshockStage == value.fromStage then
			...
			if monster then
				...
				local aftershockHealth = Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.AftershockHealth) > 0 and Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.AftershockHealth) or 0
				monster:addHealth(-monster:getHealth() + aftershockHealth, COMBAT_PHYSICALDAMAGE)
				Game.setStorageValueByKey(GlobalStorage.HeartOfDestruction.ForeshockStage, value.toStage)
				return true
			end
		end
	end
