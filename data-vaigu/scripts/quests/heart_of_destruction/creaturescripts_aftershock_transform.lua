...
function aftershockTransform.onThink(creature)
	...
	Game.setStorageValueByKey(GlobalStorage.HeartOfDestruction.AftershockHealth, creature:getHealth())
	local aftershockStage = Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.AftershockStage) > 0 and Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.AftershockStage) or 0
	...
	for index, value in pairs(monsterTable) do
		if hpPercent <= index and aftershockStage == value.fromStage then
			...
			if monster then
				...
				local foreshockHealth = Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.ForeshockHealth) > 0 and Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.ForeshockHealth) or 0
				monster:addHealth(-monster:getHealth() + foreshockHealth, COMBAT_PHYSICALDAMAGE)
				Game.setStorageValueByKey(GlobalStorage.HeartOfDestruction.AftershockStage, value.toStage)
				return true
			end
		end
	end
