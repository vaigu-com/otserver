local function createSpawnWave(stage)
	...
	Game.setStorageValueByKey(GlobalStorage.HeartOfDestruction.RuptureResonanceStage, stage + 1)
	Game.setStorageValueByKey(GlobalStorage.HeartOfDestruction.RuptureResonanceActive, 1)
end
...
function ruptureResonance.onThink(creature)
	...
	local ruptureResonanceStage = Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.RuptureResonanceStage) > 0 and Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.RuptureResonanceStage) or 0
	local resonanceActive = Game.getStorageValueByKey(GlobalStorage.HeartOfDestruction.RuptureResonanceActive)
