...
function action_water_fountain.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValueByKey(Storage.Quest.U7_4.DjinnWar.EfreetFaction.Mission02) ~= 1 then
		return true
	end

	Game.createItem(3233, 1, fromPosition)
	player:setStorageValueByKey(Storage.Quest.U7_4.DjinnWar.EfreetFaction.Mission02, 2)
