...
function boatTp.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValueByKey(Storage.TheSecretLibrary.HighDry) == 4 then
		...
		player:setStorageValueByKey(Storage.TheSecretLibrary.HighDry, 5)
		player:setStorageValueByKey(Storage.TheSecretLibrary.PinkTel, 3)
		player:setStorageValueByKey(Storage.TheSecretLibrary.Mota, 13)
		return true
	end
	return false
end
