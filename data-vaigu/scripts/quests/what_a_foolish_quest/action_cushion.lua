...
function whatFoolishCushion.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if player:getStorageValueByKey(Storage.Quest.U8_1.WhatAFoolishQuest.Questline) ~= 17 or player:getStorageValueByKey(Storage.Quest.U8_1.WhatAFoolishQuest.WhoopeeCushion) == 1 then
		return false
	end

	player:setStorageValueByKey(Storage.Quest.U8_1.WhatAFoolishQuest.WhoopeeCushion, 1)
