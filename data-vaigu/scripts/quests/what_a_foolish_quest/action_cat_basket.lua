...
function whatFoolishCat.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValueByKey(Storage.Quest.U8_1.WhatAFoolishQuest.Questline) ~= 19 or player:getStorageValueByKey(Storage.Quest.U8_1.WhatAFoolishQuest.CatBasket) == 1 then
		return false
	end

	player:setStorageValueByKey(Storage.Quest.U8_1.WhatAFoolishQuest.CatBasket, 1)
