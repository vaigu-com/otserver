...
function missionGuide.onStepIn(creature, item, position, fromPosition)
	...
	local missionState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission09)
	...
	if missionState == missionTile.state then
		...
		if missionTile.newState then
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission09, missionTile.newState)
		end
	end
	return true
end
...
function trunkChest.onUse(player, item, frompos, itemEx, topos)
	...
	if missionState >= 2 then
		...
		local chestsState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.TrollChests)
		...
		if not hasOpenedChest then
			...
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.TrollChests, chestsState + chest.id)
...
function onUsePickAtTunnelPillar(player, item, fromPosition, itemEx, toPosition)
	...
	if missionState >= 2 and missionState <= 7 and pillarId then
		local pillarsState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.TunnelPillars)
		...
		if not hasDamagedPillar then
			...
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission09, newMissionState)
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.TunnelPillars, pillarsState + pillarId)
		else
