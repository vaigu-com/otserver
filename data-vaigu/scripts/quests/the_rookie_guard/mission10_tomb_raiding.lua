...
function missionGuide.onStepIn(creature, item, position, fromPosition)
	...
	local sarcophagusState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Sarcophagus)
...
function sarcophagus.onUse(player, item, frompos, item2, topos)
	...
	if missionState >= 1 then
		...
		if sarcophagusState == -1 then
			...
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Sarcophagus, 1)
...
function unholyCryptChests.onUse(player, item, frompos, item2, topos)
	...
	local chestsState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.UnholyCryptChests)
	...
	if not hasOpenedChest then
		...
		player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.UnholyCryptChests, chestsState + chest.id)
		player:addItemEx(reward, true, CONST_SLOT_WHEREEVER)
	else
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "The " .. item:getName() .. " is empty.")
	end
