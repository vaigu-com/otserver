...
function missionGuide.onStepIn(creature, item, position, fromPosition)
	...
	local missionState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission02)
	...
	if hasUsedCatapult then
		local catapultsState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Catapults)
...
function stonePile.onUse(player, item, frompos, item2, topos)
	...
	if missionState <= 3 then
		if missionState == 1 then
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission02, 2)
		end
		-- Gather delay
		if player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.StonePileTimer) - os.time() <= 0 then
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.StonePileTimer, os.time() + 2 * 60)
...
function heavyStone.onUse(player, item, frompos, item2, topos)
	...
	if missionState >= 2 and missionState <= 3 and catapults[item2.actionid] then
		...
		if not hasUsedCatapult then
			...
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission02, missionState + 1)
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Catapults, catapultsState + catapults[item2.actionid])
