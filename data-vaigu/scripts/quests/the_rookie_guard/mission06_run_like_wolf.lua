...
function missionGuide.onStepIn(creature, item, position, fromPosition)
	...
	local missionState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission06)
...
function warWolfDenTiles.onStepIn(creature, item, position, fromPosition)
	...
	if missionState == missionTile.state then
		...
		if missionTile.newState then
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission06, missionTile.newState)
		end
	end
	return true
end
...
local function teleportBack(uid)
	...
	if player and player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission06) == 5 then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Oh no... you were too slow and the wolves caught up with you. You may try again.")
		player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission06, 4)
...
function warWolfDenBoostTiles.onStepIn(creature, item, position, fromPosition)
	...
	if missionState == 4 then
		...
		player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission06, 5)
...
warWolfDenBoostTiles:uid(50334)
...
function poacherCorpse.onUse(player, item, frompos, itemEx, topos)
	...
	if missionState == 2 then
		local corpseState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.PoacherCorpse)
		if corpseState == -1 then
			...
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.PoacherCorpse, 1)
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission06, 3)
...
function warWolfDenChest.onUse(player, item, frompos, itemEx, topos)
	...
	local chestState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.WarWolfDenChest)
	if chestState == -1 then
		local reward = Game.createItem(7876, 1)
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have found " .. reward:getArticle() .. " " .. reward:getName() .. ".")
		player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.WarWolfDenChest, 1)
		player:addItemEx(reward, true, CONST_SLOT_WHEREEVER)
	else
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "The " .. item:getName() .. " is empty.")
	end
