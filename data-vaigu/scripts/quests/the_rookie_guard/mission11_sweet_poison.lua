...
function missionGuide.onStepIn(creature, item, position, fromPosition)
	...
	local missionState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission11)
	...
	if missionState == missionTile.state then
		...
		if missionTile.newState then
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission11, missionTile.newState)
		end
	end
	return true
end
...
function specialFlask.onUse(player, item, frompos, item2, topos)
	...
	if missionState == 2 and item2.itemid == 5989 then
		...
		player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission11, 3)
