...
function missionGuide.onStepIn(creature, item, position, fromPosition)
	...
	local missionState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.Mission07)
	-- Skip if not was started or finished
	if missionState == -1 or missionState == 2 then
		return true
	end
	local missionTile = missionTiles[item.actionid]
	local libraryChestState = player:getStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.LibraryChest)
...
function treasureChest.onUse(player, item, frompos, item2, topos)
	...
	if missionState == 1 then
		...
		if libraryChestState == -1 then
			...
			player:setStorageValueByKey(Storage.Quest.U9_1.TheRookieGuard.LibraryChest, 1)
