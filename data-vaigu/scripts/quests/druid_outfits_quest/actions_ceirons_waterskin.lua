...
function actions_ceirons_waterskin.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if target.itemid == 5662 and target.uid == 40096 and player:getStorageValueByKey(Storage.Quest.U7_8.DruidOutfits.CeironsWaterskin) ~= 1 then
		toPosition:sendMagicEffect(CONST_ME_LOSEENERGY)
		item:transform(5939)
		player:setStorageValueByKey(Storage.Quest.U7_8.DruidOutfits.CeironsWaterskin, 1)
