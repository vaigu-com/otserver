...
function ragingMage1.onDeath(creature, corpse, killer, mostDamageKiller, unjustified, mostDamageUnjustified)
	...
	if killer:getStorageValueByKey(playerDoubleMageKill) < 2 and not mostDamageKiller:hasAchievement("Mageslayer") and Game.getStorageValueByKey(yielothaxKillStorage) < 2000 then
		...
		mostDamageKiller:setStorageValueByKey(playerDoubleMageKill, mostDamageKiller:getStorageValueByKey(playerDoubleMageKill) + 1)
		mostDamageKiller:addAchievement("Mageslayer")
	elseif Game.getStorageValueByKey(yielothaxKillStorage) < 2000 then
		creature:say("MWAAAHAHAAA!! NO ONE!! NO ONE CAN DEFEAT MEEE!!!", TALKTYPE_MONSTER_YELL, false, nil, Position(33143, 31527, 2))
		Game.createMonster("Energized Raging Mage", Position(33142, 31529, 2))
	elseif Game.getStorageValueByKey(yielothaxKillStorage) > 1999 then
		...
		Game.setStorageValueByKey(yielothaxKillStorage, 0)
		Game.setStorageValueByKey(enoughKills, 1)
		local t, spectator = Game.getSpectators(creature:getPosition(), false, false, 10, 10, 10, 10)
		if #t ~= nil then
			for i = 1, #t do
				spectator = t[i]
				if spectator:isPlayer() and spectator:getStorageValueByKey(673004) < 250 then
