...
function geomanticCharges.onStepIn(creature, item, position, fromPosition)
	...
	local tasksLoaded = {}
	if not table.contains({ -1, 8 }, player:getStorageValueByKey(SPIKE_MIDDLE_NEST_MAIN)) then
		tasksLoaded["NEST"] = true
	end
	if player:getStorageValueByKey(SPIKE_MIDDLE_CHARGE_MAIN) == 1 then
	...
	if tasksLoaded["CHARGE"] then
		...
		player:setStorageValueByKey(SPIKE_MIDDLE_CHARGE_MAIN, 2)
