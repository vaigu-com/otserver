...
function rewardRoomText.onStepIn(creature, item, position, fromPosition)
	...
	if not player or player:getStorageValueByKey(Storage.TheInquisition.RewardRoomText) == 1 then
		return true
	end

	player:setStorageValueByKey(Storage.TheInquisition.RewardRoomText, 1)
