		...
		Quests[NextQuestId()] = {
			...
			missions = {
				[Storage.WayOfTheDruid.CouncilOfDruids] = {
					...
					states = {
						[1] = "You helped the first druid. This is the beginning of your journey to become one of them. Find the remaining druid, and provide them with your help.",
						[2] = "The Mother Nature is watching, and she is cherishing as you helped two of the druids.",
						[3] = "Your efforts induced The Mother Nature to let you gather water from well at the great tree. You have helped three druids so far.",
						[4] = "Your kindness goes a long way, you helped four druids so far. You are now able to gather water from the great tree well.",
						[5] = "You helped all the druids, and you have been introduced to their Council. In addition to being able to gather water from the well you can also harvest a blossom of the Griffinclaw plant, which grows inside the great tree.",
					},
				},
				...
				[Storage.WayOfTheDruid.SecretIngredient] = {
					...
					states = {
						...
						[2] = "Ornuld wishes to have a one wyrm egg. If you manage to do this favor, he will reward you with the medicine.",
						[3] = "You found the wyrm's egg, hand it to Ornuld.",
						[4] = "Ornuld gave you the medicine in exchange for the wyrm's egg. Report to Mundral now.",
						[5] = "Turn out that Mundral belongs to the Druid Council. He put in a good word to his druid friends and he gifted you with bamboo leaves.",
					},
				},
	...
	:State(
		...
		QuestFactory.Dialog("Sand Nigger", {
			...
			[{ "map", "mapa" }] = {
				...
				rewards = {
					{ id = 22107, aid = Storage.WayOfTheDruid.SandniggerMap },
				},
				nextState = {
					[Storage.WayOfTheDruid.RudeEviction] = _38f,
				},
			},
		})
	)
	...
	:State(
		...
		QuestFactory.Dialog("Estep", {
			...
			[{ "yes", "tak" }] = {
				...
				nextState = {
					[Storage.WayOfTheDruid.TakenBenek] = PH_STATE,
				},
			},
		})
	)
	:State(
		...
		QuestFactory.Script(function()
			...
			function knife.onUse(player, item, fromPosition, target, toPosition, isHotkey)
				if target == nil or target:getId() ~= 12369 or target:getActionId() ~= Storage.WayOfTheDruid.BenekKnife then
					return false
				end

				toPosition:sendMagicEffect(CONST_ME_POFF)
				player:UpdateStorages({
					[Storage.WayOfTheDruid.TakenBenek] = PH_STATE,
				})
			end
		...
		QuestFactory.StartupItems({
			{ id = 12369, pos = { 5810, 1292, 7 }, aid = Storage.WayOfTheDruid.BenekKnife },
		})
	)
	:State(
		PH_STATE,
	...
	:State(
		...
		QuestFactory.StartupItems({
			...
			{ id = 14098, pos = { 6622, 567, 3 }, aid = Storage.WayOfTheDruid.WyrmEgg },
			{
				...
				rewards = {
					{ id = 18996, aid = Storage.WayOfTheDruid.WyrmEgg },
				},
				spawnMonstersOnSuccess = {
	...
	:State(
		...
		QuestFactory.Dialog("Ornuld", {
			[{ "mission", "misja" }] = {
				...
				expReward = 60000,
				rewards = {
					{ id = 8819, aid = Storage.WayOfTheDruid.OrnuldMedicine },
				},
				nextState = {
					[Storage.WayOfTheDruid.SecretIngredient] = _38f,
				},
