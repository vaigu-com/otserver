...
quest
				...
				[{ "mission", "misja" }] = {
					...
					specialRequirements = {
						requirement = function(context)
							local completedDruidQuests = 0
							for key, value in pairs({ druidQuests }) do
								if context.player:getStorageValueByKey(value) == MISSION_FINISHED then
									completedDruidQuests = completedDruidQuests + 1
								end
							end
							return completedDruidQuests >= 3
						end,
	...
	:State(function()
		return { min = QuestState.WayOfTheDruid.CouncilOfDruids.HelpedTwo, max = QuestState.WayOfTheDruid.CouncilOfDruids.HelpedFour }, QuestFactory.Dialog({ "Radaghast the brown", "Sand Nigger", "Malfurion", "Mundral", "Estep" }, {
			[{ "druid", "druids", "krag", "circle", "druidow", "druidzi" }] = {
		...
		return QuestState.WayOfTheDruid.RudeEviction.FindSandniggerSpell,
			QuestFactory.Script(function()
				...
				function map.onUse(player, item, fromPosition, target, toPosition, isHotkey)
					...
					local announcement = player:Localizer(LOCALIZERS.Universal):Context({ dir = dir }):Get("GO_IN_DIRECTION")
