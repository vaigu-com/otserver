...
local function extractItemData(item)
	...
	return { id = id, count = count, aid = actionid, uid = uniqueid }
end
...
local function positionChestRewards(chest)
...
local function isCooldownExpired(player, chest)
	...
	return os.time() > player:getStorageValue(storage) -- ToDo: change to kv therefore storage key should be string and value can be string (enum)
end
...
function positionChest.onUse(player, chest, fromPosition, target, toPosition, isHotkey)
	...
	local rewards = positionChestRewards(chest)
