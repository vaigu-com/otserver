...
quest
	...
	:Constant(function()
		...
		DESERT_QUEST_ONE_SKIPS = {
			[Storage.DesertQuestOne.QuestState.VocPuzzle] = Vector(71, 88, 0), -- main hub -> past 4 mwalls
			[Storage.DesertQuestOne.QuestState.BasinPuzzle] = Vector(-9, -4, 1), -- brazier -> key shaft
			[Storage.DesertQuestOne.QuestState.CastlePuzzle] = Vector(-21, 69, 2), -- void castle -> ice skating
			[Storage.DesertQuestOne.QuestState.BreakIcePuzzle] = Vector(-27, 110, 2), -- dont break ice -> monster guiding
			[Storage.DesertQuestOne.QuestState.GuidingWispPuzzle] = Vector(17, 160, 1), -- monster guiding -> water flow
			[Storage.DesertQuestOne.QuestState.WaterFlowPuzzle] = Vector(-59, 148, 2), -- water flow -> movable tiles
			[Storage.DesertQuestOne.QuestState.MoveableTilesPuzzle] = Vector(-39, -9, 1), -- movable tiles -> rainbow room
		}
		...
		return ANY_STATE,
			...
			QuestFactory.Script(function(missionState)
				local vectorToMessage = {
					...
					[Vector(-59, -18, 1)] = "Below I'm gonna present how I came to conclusions on the previous page. On the starting signs (with colored floor) there are tables with numbers 0, 1, 2 and letter x. The letter x means a center position. The numbers mean how many chains are going across a given tile, relative to the x tile. \n\nAs I mentioned on earlier pages, tiles have 0, 1 or 2 chains going over them. Usually, there is only 1 chain going over a tile. \n\nI tried to find a place where the surrounding tiles would have same numbers of chains as on one of the tables on signs.\n\n~Pages 89, 90 of Eustachy Wiertary journal.",
					[Vector(-53, -33, 1)] = "I noticed that there is a total of 7 signs in this room, each with different colored tiles. Each sign points to the next sign, which is pointing to another one, ultimately ending on the last sign with white floor. This way someone set 7 different chains of signs. \n\nI also noticed that every chain goes over black marble tiles. However, not every black marble has a chain going across it, and some have up to two chains going across. This knowledge might be essential later.\n\n~Pages 85, 86 of Eustachy Wiertara journal.",
				...
				function bookUse.onUse(player, item, fromPosition, target, toPosition, isHotkey)
					...
					displayBookContent(player, bookUse)
					return true
				end
			...
			QuestFactory.Script(function(missionState)
				...
				local function playerCompletedCurrentMysteriando(player, currentActionid)
					return player:getStorageValueByKey(currentActionid) == 1
				end

				local function playerCompletedNoMysteriandos(player)
					return player:getStorageValueByKey(questline) == -1
				end

				local function giveMysteriandoCredit(player, aid, nextQuestlineValue)
					player:setStorageValueByKey(questline, nextQuestlineValue)
					player:setStorageValueByKey(aid, 1)
				...
				function TryProgressDesertQuest(creature, itemOrAid)
					...
					if playerCompletedNoMysteriandos(player) then
						player:setStorageValueByKey(questline, 0)
					end
			...
			QuestFactory.Script(function(missionState)
				...
				function gravestone.onUse(player, item, fromPosition, target, toPosition, isHotkey)
					...
					addEvent(function()
						...
						portal:setActionId(Storage.DesertQuestOne.QuestState.CastlePuzzle)
					end, 1000 * 5)
					addEvent(function()
						portal:remove()
