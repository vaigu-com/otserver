		...
		return QuestState.PathOfTheUndead.Mission02.VisitMagicalCircles,
			...
			QuestFactory.Script(function(missionState)
				...
				local function GetUncompletedCirclesCount(player)
					local storageVal = player:getStorageValueByKey(circleStorage)
					local length = 0
				...
				function step_tile.onStepIn(player, item, toPosition, fromPosition)
					...
					local questlineStorageVal = player:getStorageValueByKey(questlineStorage)
					...
					if storageVal < 0 then
						player:setStorageValueByKey(circleStorage, 0)
					end

					local addend = idToStorageVal[item:getId()]
					if not IsFactor(storageVal, addend) then
						player:setStorageValueByKey(circleStorage, player:getStorageValueByKey(circleStorage) + addend)
				...
				step_tile:key(Storage.PathOfTheUndead.Circles)
	...
	:State(function()
		return QuestState.PathOfTheUndead.Mission03.DefeatUndeadKing,
			...
			QuestFactory.Script(function(missionState)
				...
				step_tile:key(Storage.PathOfTheUndead.RitualLocus)
				step_tile:type("stepin")
				step_tile:register()
			end),
			QuestFactory.Script(function(missionState)
