...
function bait.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.QuestLine) == 1 then
		if target.itemid == 3496 then -- crane
			if player:getStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.Bait) == 1 then
				player:say("The bait is already set. Go up to the lookout and check the telescope!", TALKTYPE_MONSTER_SAY)
			else
				item:remove(1)
				toPosition:sendMagicEffect(CONST_ME_WATERSPLASH)
				player:setStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.Bait, 1)
			end
		end
	end
	return true
...
function telescope.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.QuestLine) == 1 then
		...
		if player:getStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.SuccessSwitch) == 1 then
		...
		elseif player:getStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.Direction) > 0 then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Oops! You lost it.")
			player:setStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.SuccessSwitch, 0)
			player:setStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.Direction, 0)
		elseif randAppear <= 4 then -- 40% nothing
			...
			if randAppear >= 5 and randAppear <= 8 then -- 40% nothing
				...
				player:setStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.Direction, randDirection)
				if randBait == 2 then
					player:setStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.Bait, 0)
				end
			elseif randAppear == 9 then -- 10% nothing
				...
				player:setStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.Direction, 4)
				if randBait == 2 then
					player:setStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.Bait, 0)
				end
			elseif randAppear == 10 then -- 10% success
				...
				player:setStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.QuestLine, 2)
				player:setStorageValueByKey(Storage.Quest.U8_2.TheHuntForTheSeaSerpent.Access, 1)
			end
		end
	end
	return true
