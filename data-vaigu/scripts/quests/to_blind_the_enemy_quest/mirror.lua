...
function mirror.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValueByKey(Storage.TibiaTales.ToBlindTheEnemy.Questline) == 1 then
		if target:getActionId() == 4001 and target.itemid == 132 then
			...
			if itemToRemove then
				...
				player:setStorageValueByKey(Storage.TibiaTales.ToBlindTheEnemy.Questline, 2)
