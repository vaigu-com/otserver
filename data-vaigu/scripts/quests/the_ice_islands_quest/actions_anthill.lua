...
function iceAntHill.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target.itemid == 1079 and item.itemid == 7243 then
		if player:getStorageValueByKey(Storage.Quest.U8_0.TheIceIslands.Questline) == 6 then
			toPosition:sendMagicEffect(CONST_ME_GROUNDSHAKER)
			player:setStorageValueByKey(Storage.Quest.U8_0.TheIceIslands.Mission03, 2) -- Questlog The Ice Islands Quest, Nibelor 2: Ecological Terrorism
	...
	elseif target.itemid == 4940 and item.itemid == 7244 and toPosition.x == mast.x and toPosition.y == mast.y and toPosition.z == mast.z then
		if player:getStorageValueByKey(Storage.Quest.U8_0.TheIceIslands.Questline) == 6 then
			...
			player:setStorageValueByKey(Storage.Quest.U8_0.TheIceIslands.Questline, 7)
			player:setStorageValueByKey(Storage.Quest.U8_0.TheIceIslands.Mission03, 3) -- Questlog The Ice Islands Quest, Nibelor 2: Ecological Terrorism
