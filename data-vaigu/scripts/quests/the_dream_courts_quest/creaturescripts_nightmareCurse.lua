...
local function resetArea()
	...
	for _, p in pairs(spectators) do
		...
		if player then
			player:setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.DreamScar.LastBossCurse, 0)
		end
	end
end
...
function creaturescripts_nightmareCurse.onThink(creature, interval)
	...
	local stage = player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.NightmareCurse)
	...
	if stage == 1 and not player:getCondition(CONDITION_OUTFIT, spectre) then
		...
		addEvent(function(cid)
			...
			if p then
				p:setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.NightmareCurse, 2)
			end
	...
	elseif stage == 2 and not player:getCondition(CONDITION_OUTFIT, phantasm) then
		...
		addEvent(function(cid)
			...
			if p and p:getCondition(CONDITION_OUTFIT, phantasm) then
				...
				p:setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.NightmareCurse, -1)
