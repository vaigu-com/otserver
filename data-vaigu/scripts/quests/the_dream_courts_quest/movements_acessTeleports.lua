...
function movements_acessTeleports.onStepIn(creature, item, position, fromPosition)
	...
	if item:getPosition() == nightmareTeleport then
		if player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.DreamScar.BossCount) >= 5 then
			if player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.DreamScar.NightmareTimer) > os.time() then
	...
	for _, k in pairs(default) do
		if k.itemPosition == iPos then
			if player:getStorageValueByKey(k.neededStorage) >= k.value then
	...
	if iPos == dreamScarTeleport then
		if player:getStorageValueByKey(permission) >= 1 then
			for i = 1, #dreamScar do
				if os.date("%A") == dreamScar[i].day then
					if player:getStorageValueByKey(dreamScar[i].storageTimer) > os.time() then
