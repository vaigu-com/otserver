		...
		if cName == k.bossName then
			for pid, _ in pairs(creature:getDamageMap()) do
				...
				if attackerPlayer then
					if attackerPlayer:getStorageValueByKey(k.storageQuestline) <= k.maxValue then
						attackerPlayer:setStorageValueByKey(k.storageQuestline, attackerPlayer:getStorageValueByKey(k.storageQuestline) + 1)
					end
					attackerPlayer:setStorageValueByKey(k.storageTimer, os.time() + 20 * 60 * 60)
				end
			end

			if cName:lower() == "alptramun" then
				Game.setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.DreamScarGlobal.AlptramunSummonsKilled, 0)
			end
		end
	end

	local summonsKilled = Game.getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.DreamScarGlobal.AlptramunSummonsKilled)
	...
	for _, k in pairs(alptramunSummons) do
		if cName:lower() == k.name then
			if summonsKilled >= k.minValue and summonsKilled <= k.maxValue then
				Game.setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.DreamScarGlobal.AlptramunSummonsKilled, summonsKilled + 1)
			end
		end
	end
