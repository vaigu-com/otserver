...
local function resetArea()
	...
	for _, p in pairs(spectators) do
		...
		if player then
			...
			if player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.DreamScar.LastBossCurse) >= 1 then
	...
	elseif item.itemid == 29276 then
		if player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.DreamScar.LastBossCurse) < 1 then
			if (target ~= player) and target:isPlayer() then
				if target:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.NightmareCurse) >= 1 then
					target:setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.NightmareCurse, 0)
					...
					player:setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.NightmareCurse, 1)
					...
					local j = resetArea()
					if j then
						Game.setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.DreamScarGlobal.LastBossCurse, 0)
					end
				else
					return true
				end
