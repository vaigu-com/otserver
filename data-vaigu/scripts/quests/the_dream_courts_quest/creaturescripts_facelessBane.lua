...
function creaturescripts_facelessBane.onThink(creature, interval)
	...
	local lifes = creature:getStorageValueByKey(storage)
	local percentageHealth = (creature:getHealth() / creature:getMaxHealth()) * 100

	if lifes <= 3 then
		if lifes < 0 then
			creature:setStorageValueByKey(storage, 0)
		end
		if percentageHealth <= 20 then
			...
			creature:setStorageValueByKey(storage, lifes + 1)
			Game.setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.BurriedCatedral.FacelessLifes, 0)
		end
	end
