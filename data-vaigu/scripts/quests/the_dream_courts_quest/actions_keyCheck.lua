...
function actions_keyCheck.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local isInQuest = player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.HauntedHouse.Tomb)

	if player:getStorageValueByKey(storage) == 1 and isInQuest < 1 then
		...
		elseif iPos == pillar[3] then
			...
			player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
			player:setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.HauntedHouse.Tomb, 1)
			if player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.HauntedHouse.Tomb) == 1 and player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.HauntedHouse.Cellar) == 1 and player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.HauntedHouse.Temple) == 1 then
