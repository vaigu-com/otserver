...
function actions_sequenceSkulls.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if player:getStorageValueByKey(sequenceSkulls) < 0 then
		player:setStorageValueByKey(sequenceSkulls, 0)
	end

	local tPos = target:getPosition()

	if player:getItemCount(bookId) >= 1 and player:getStorageValueByKey(sequenceSkulls) < 3 then
		for _, skull in pairs(sequence) do
			if tPos == skull.position and skull.id == item.itemid then
				if player:getStorageValueByKey(sequenceSkulls) == skull.number then
					player:setStorageValueByKey(sequenceSkulls, player:getStorageValueByKey(sequenceSkulls) + 1)
					tPos:sendMagicEffect(CONST_ME_MAGIC_GREEN)
					player:sendTextMessage(MESSAGE_EVENT_ADVANCE, skull.msg)
					item:remove(1)
					if player:getStorageValueByKey(sequenceSkulls) == 3 then
						player:setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.TheSevenKeys.SkullRoom, ACCESS_GRANTED)
					end
				end
			end
		end
