...
function silencerExcretion.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if isInArray(guzzleShrines, target.uid) then
		if getStorageValueByKey(Storage.MawhawkState) >= 20 then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Mawhawk got spawned already.")
			return false
		end

		if getStorageValueByKey(target.uid) >= os.time() then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "This one still smells really bad..")
			return false
		end

		if getStorageValueByKey(Storage.MawhawkState) < 0 then
			setStorageValueByKey(Storage.MawhawkState, 0)
		end

		item:remove(1)
		toPosition:sendMagicEffect(CONST_ME_SMALLCLOUDS)
		setStorageValueByKey(Storage.MawhawkState, getStorageValueByKey(Storage.MawhawkState) + 1)
		setStorageValueByKey(target.uid, os.time() + 1 * 60)
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You desecrated " .. getStorageValueByKey(Storage.MawhawkState) .. " out of 20 shrines to spawn a Mawhawk.")
