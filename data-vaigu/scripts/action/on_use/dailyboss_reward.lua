...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local container = Container(player:AddCustomItem({id = 8861}):getUniqueId())
	...
	for i = 1, #reward.items do
		...
		if items.chance then
			if items.chance >= math.random(1, 100) then
				if items.levelRestriction then
					if player:getLevel() >= 25 then -- dont add rare items below 25 lvl (prevent noobchars)
						container:addItem(items.itemId[math.random(#items.itemId)], items.count or 1)
					end
				else
					container:addItem(items.itemId[math.random(#items.itemId)], items.count or 1)
				end
			...
			container:addItem(items.itemId, items.count or 1)
		end
	end
