...
<<<<<<<< HEAD:data-vaigu/scripts/action/on_use/mounts/ursagrodon.lua
		if player:getStorageValueByKey(config.item4) > 0 then
========
		if player:getStorageValue(config.storageKey) > 0 then
>>>>>>>> c3bafd85d37e25814e6a6548e2252b64f9f7f33a:data-otservbr-global/scripts/quests/tinder_box_quest_chyllfroest/actions_ursagrodon.lua
		...
		if rand <= config.percentage then
			...
			elseif target.itemid == config.item3 then
				...
				player:addMount(38)
<<<<<<<< HEAD:data-vaigu/scripts/action/on_use/mounts/ursagrodon.lua
				player:setStorageValueByKey(config.item4, 1)
========
