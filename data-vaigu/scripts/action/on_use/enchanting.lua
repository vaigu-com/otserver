...
local vocationToHuntingGroundDestination = {
...
local function handleElementalSpheresHuntingGroundMachine(player, gem)
	if player:getStorageValueByKey(Storage.ElementalSphere.QuestLine) <= 0 then
		return false
	end
	gem:remove(1)
	local toPosition = vocationToHuntingGroundDestination[player:getVocation():getBase():getId()]
...
function enchanting.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target:getKey() == Storage.ElementalSphere.HuntingGroundMachine then
		return handleElementalSpheresHuntingGroundMachine(player, item)
	end
