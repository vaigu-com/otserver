local freeBlessMaxLevelSharp = 35

local creatureevent = CreatureEvent("freebless")
function creatureevent.onLogin(player)
	if player:getLevel() <= freeBlessMaxLevelSharp then
		for i = 2, 6 do
		...
		if getPlayerLanguage(player) == "PL" then
			...
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You receive free blessings until " .. freeBlessMaxLevelSharp .. " level.")
		end
