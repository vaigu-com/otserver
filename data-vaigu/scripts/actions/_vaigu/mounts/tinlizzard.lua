...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 13292 and itemEx.itemid == 13306 then
		if player:getStorageValueByKey(Storage.Tinlizzard) < 1 then
			if n < broken then
				...
				player:setStorageValueByKey(Storage.Tinlizzard, 1)
				if player:getStorageValueByKey(Storage.Tinlizzard) == 1 then
		...
		elseif player:getStorageValueByKey(Storage.Tinlizzard) == 1 then
