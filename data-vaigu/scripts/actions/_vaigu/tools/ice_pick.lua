...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if target.itemid == 6966 then
		if player:getStorageValueByKey(Storage.IcePickIcicle) >= os.time() then
			return player:say("Na dzis juz wystarczy..", TALKTYPE_MONSTER_SAY)
		end
		if player:getStorageValueByKey(Storage.UnwantedGuests.ShitfacedBarbarians) == 2 then
			...
			elseif roll >= 61 and roll <= 85 then
				...
				toPosition:sendMagicEffect(38)
				player:setStorageValueByKey(Storage.IcePickIcicle, os.time() + 20 * 3600) --20h
