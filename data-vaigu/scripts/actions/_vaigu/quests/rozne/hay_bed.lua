...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if player:getStorageValueByKey(Storage.Yalahar.MiloscAriela) ~= 6 then -- czy ktos juz bral to i czy ma aktywny od postmana
		return false
	end

	if hay_bed_id == item.itemid then
		if player:getStorageValueByKey(Storage.Yalahar.Haybed) ~= 1 then
			if player:getItemCount(19133) > 0 then -- czy ma eliksir przy sobie
				...
				player:setStorageValueByKey(item.uid, 1) -- uaktualnienie uid lozka na 1
			end
		end
	end
	return true
