...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if player:getStorageValue(Storage.WayOfTheDruid.OrnuldMedicine) < 1 then -- Jak ktos nie ma questa
		return false
	end
	if wyrm_egg == item.itemid then
		if player:getStorageValue(Storage.WayOfTheDruid.OrnuldMedicine) < 2 then
			player:AddCustomItem({ id = 18996,addToStore = true }) 
			player:setStorageValue(Storage.WayOfTheDruid.OrnuldMedicine, 2)
