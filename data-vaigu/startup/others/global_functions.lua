...
local function addFerumbrasAscendantReward(player, target, toPosition)
	...
	if target.itemid == 22872 and target.actionid == 53803 then
		if player:getStorageValueByKey(Storage.FerumbrasAscension.Ring) >= 1 then
			return false
		end

		player:AddCustomItem({id = 22170, count = 1})
		player:setStorageValueByKey(Storage.FerumbrasAscension.Ring, 1)
	end
end
...
function onDestroyItem(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if math.random(1, 80) <= (watt and watt > 10 and watt or 10) then
		...
		if target.itemid == 233 then
			local eggStorage = player:getStorageValueByKey(Storage.TibiaTales.AgainstTheSpiderCult)
			if eggStorage >= 1 and eggStorage < 5 then
				player:setStorageValueByKey(Storage.TibiaTales.AgainstTheSpiderCult, math.max(1, eggStorage) + 1)
			end
...
function onUseShovel(player, item, fromPosition, target, toPosition, isHotkey)
		...
		if amuletId then
			if player:getStorageValueByKey(Storage.Quest.U10_55.Dawnport.TheLostAmulet) == 1 then
				...
				elseif rand > 95 then
					...
					player:AddCustomItem({id = 3492, count = amount})
	...
	elseif target.actionid == 4654 and player:getStorageValueByKey(Storage.GravediggerOfDrefia.Mission49) == 1 and player:getStorageValueByKey(Storage.GravediggerOfDrefia.Mission50) < 1 then
		-- Gravedigger Quest
		...
		player:setStorageValueByKey(Storage.GravediggerOfDrefia.Mission50, 1)
	elseif target.actionid == 4668 and player:getStorageValueByKey(Storage.GravediggerOfDrefia.Mission69) == 1 and player:getStorageValueByKey(Storage.GravediggerOfDrefia.Mission70) < 1 then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "A torn scroll piece emerges. Probably gnawed off by rats.")
		player:getPosition():sendMagicEffect(CONST_ME_POFF)
		player:AddCustomItem({id = 18933, count = 1})
		player:setStorageValueByKey(Storage.GravediggerOfDrefia.Mission70, 1)
	...
	elseif target.itemid == 103 and target.actionid == 4205 then
		if player:getStorageValueByKey(Storage.TibiaTales.IntoTheBonePit) ~= 1 then
			return false
		end
...
function onUsePick(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif target.itemid == 22075 then
		-- Grimvale quest
		if player:getStorageValueByKey(Storage.Grimvale.SilverVein) < os.time() then
			...
			player:setStorageValueByKey(Storage.Grimvale.SilverVein, os.time() + 2 * 60)
		else
			player:sendTextMessage(
				MESSAGE_EVENT_ADVANCE,
	...
	elseif target.itemid == 1791 then
		...
		elseif target.actionid == 40031 then
			-- naginata quest
			local stoneStorage = Game.getStorageValueByKey(Storage.NaginataStone)
			if stoneStorage ~= 5 then
				Game.setStorageValueByKey(Storage.NaginataStone, math.max(0, stoneStorage) + 1)
			elseif stoneStorage == 5 then
				target:remove(1)
				Game.setStorageValueByKey(Storage.NaginataStone)
			end
			toPosition:sendMagicEffect(CONST_ME_POFF)
	...
	elseif target.actionid == 50127 then
		...
		if player:getStorageValueByKey(Storage.QuestChests.FirewalkerBoots) == 1 then
			return false
		end
		target:remove(1)
	...
	elseif target.itemid == 2071 then
		...
		if player:getStorageValueByKey(Storage.TibiaTales.JackFutureQuest.Statue) ~= 1 then
			return false
		end
		...
		if toPosition == Position(33277, 31754, 7) then
			if player:getStorageValueByKey(Storage.TibiaTales.JackFutureQuest.Statue) == 1 then
				...
				player:setStorageValueByKey(Storage.TibiaTales.JackFutureQuest.Statue, 2)
				player:setStorageValueByKey(Storage.TibiaTales.JackFutureQuest.QuestLine, player:getStorageValueByKey(Storage.TibiaTales.JackFutureQuest.QuestLine) + 1)
...
function onUseCrowbar(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif target.actionid == 100 then
		if target.itemid == 3501 then
			-- Postman quest
			if player:getStorageValueByKey(Storage.Postman.Mission02) == 1 then
				player:setStorageValueByKey(Storage.Postman.Mission02, 2)
...
local function addedAnywhere(parent, topParent , value)
	if parent:isTile() then
		return false
	end
	if parent:addItem(value.item_id, 1) then
		return true
	end
	if topParent:addItem(value.item_id, 1) then
		return true
	end
	return false
end
	...
	for index, value in pairs(itemsGrinder) do
		if item.itemid == index then
			...
			if topParent.isItem and (not topParent:isItem() or topParent.itemid ~= 470) then
				...
				if addedAnywhere(parent, topParent , value) then
					item:remove(1)
