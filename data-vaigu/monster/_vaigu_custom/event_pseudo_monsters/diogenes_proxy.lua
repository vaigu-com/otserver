local mType = Game.createMonsterType("DiogenesProxy")
local monster = {}
...
local fakeMonsterStartup = GlobalEvent("DiogenesProxyStartup")
function fakeMonsterStartup.onStartup()
	...
	Game.createMonster("DiogenesProxy", Position(6239, 1025, 5))
	return true
end
...
local diogenesBed = Look()
function diogenesBed.onLook(player)
	player:sendTextMessage(MESSAGE_LOOK, "You see a hammock.\nDiogenes is sleeping here.")
	return DONT_SHOW_ONLOOK
end
diogenesBed:key(Storage.Diogenes.Bed)
diogenesBed:register()
