...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "mission", "misja", "misje" }, message) then
		if player:getStorageValue(Storage.ToCarryThePigs.HalfTurnKick) == 3 then
			if player:getItemCount(22160) > 0 then
				if player:getLevel() >= 40 then
					player:removeItem(22160, 1) -- zabieramy Liste
					player:setStorageValue(Storage.ToCarryThePigs.HalfTurnKick, 4) -- konczymy klesta
		...
		elseif player:getStorageValue(Storage.ToCarryThePigs.HalfTurnKick) == 1 then
			if player:getItemCount(22160) > 0 then
				if player:getStorageValue(Storage.ToCarryThePigs.ListTaken) == 1 then
					player:setStorageValue(Storage.ToCarryThePigs.HalfTurnKick, 2)
		...
		elseif player:getStorageValue(Storage.ToCarryThePigs.HalfTurnKick) < 1 then
			if player:getStorageValue(Storage.StickyBeginning.WoodDelivery) == 6 then
				...
				player:setStorageValue(Storage.ToCarryThePigs.HalfTurnKick, 1)
				npcHandler:say(getPlayerLanguage(player) == "PL" and {
