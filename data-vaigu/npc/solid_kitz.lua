...
local function creatureSayCallback(npc, creature, type, message)
	...
	-- ============= Fan Arnolda =================
	if table.contains({ "mission", "misja", "pomoc", "help" }, message) and player:getStorageValue(Storage.ToCarryThePigs.KitzDominando) == 2 and player:getItemCount(9378) > 0 then
		if player:getLevel() >= 20 then
			player:setStorageValue(Storage.ToCarryThePigs.KitzDominando, 3) -- wykonana misja
	...
	elseif table.contains({ "mission", "misja", "pomoc", "help" }, message) and player:getStorageValue(Storage.ToCarryThePigs.KitzDominando) < 1 then
		if player:getStorageValue(Storage.StickyBeginning.WoodDelivery) == 6 then
			...
			player:setStorageValue(Storage.ToCarryThePigs.KitzDominando, 1)
			npcHandler:say(getPlayerLanguage(player) == "PL" and {
	...
	elseif table.contains({
		...
		"camp",
		"oboz",
	}, message) and player:getStorageValue(Storage.TheaterOfCheapThrills.Mission01) == 17 then
	...
	elseif table.contains({ "yes", "tak" }, message) and player:getStorageValue(Storage.TheaterOfCheapThrills.Mission01) == 17 and npcHandler:getTopic(playerId) == 1 then
		player:setStorageValue(Storage.TheaterOfCheapThrills.Mission01, 18)
	...
	elseif table.contains({ "no", "nie" }, message) and player:getStorageValue(Storage.TheaterOfCheapThrills.Mission01) == 17 and npcHandler:getTopic(playerId) == 1 then
		player:setStorageValue(Storage.TheaterOfCheapThrills.Mission01, 18)
		player:setStorageValue(Storage.TheaterOfCheapThrills.Mission06, 4)
