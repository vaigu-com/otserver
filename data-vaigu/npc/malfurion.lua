...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "yes", "tak" }, message) then
		if player:getStorageValue(Storage.WayOfTheDruid.SingingCrystal) < 1 and npcHandler:getTopic(playerId) == 1 then
			player:setStorageValue(Storage.WayOfTheDruid.SingingCrystal, 1)
	...
	elseif table.contains({ "mission", "misja", "pomoc", "help" }, message) then
		if player:getStorageValue(Storage.WayOfTheDruid.SingingCrystal) == 1 then
			if player:getStorageValue(Storage.WayOfTheDruid.CouncilOfDruids) == 4 then
				npcHandler:say(getPlayerLanguage(player) == "PL" and {
				...
				if player:getItemCount(7290) > 0 then
					...
					player:setStorageValue(Storage.WayOfTheDruid.SingingCrystal, 3) -- skonczona misja
		...
		elseif player:getStorageValue(Storage.WayOfTheDruid.SingingCrystal) < 1 then
