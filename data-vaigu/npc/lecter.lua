...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "skory", "skin", "skins" }, message) and player:getStorageValue(Storage.SilenceOfTheLambs.FashionistaClothes) == 1 then
	...
	if table.contains({ "mieso", "mission", "misja" }, message) then
		if player:getStorageValue(Storage.SilenceOfTheLambs.TestOfStrength) < 1 then
			...
			player:setStorageValue(Storage.SilenceOfTheLambs.TestOfStrength, 1) -- zaczynamy klesta i misje 1
			npcHandler:setTopic(playerId, 1)
		end
		if player:getStorageValue(Storage.SilenceOfTheLambs.TestOfStrength) == 2 and player:getStorageValue(Storage.SilenceOfTheLambs.FashionistaClothes) < 1 then
			...
			player:setStorageValue(Storage.SilenceOfTheLambs.FashionistaClothes, 1) -- zaczynamy misje 2
			npcHandler:setTopic(playerId, 1)
		end
		if player:getStorageValue(Storage.SilenceOfTheLambs.TestOfStrength) == 1 and npcHandler:getTopic(playerId) < 1 then
			if player:getItemCount(9685) > 4 then
				if player:getLevel() >= 15 then
					...
					player:setStorageValue(Storage.SilenceOfTheLambs.TestOfStrength, 2) -- konczymy ta czesc questa
				else
		...
		if player:getStorageValue(Storage.SilenceOfTheLambs.FashionistaClothes) == 1 and npcHandler:getTopic(playerId) < 1 then
			if
			...
			then
				...
				player:setStorageValue(Storage.SilenceOfTheLambs.FashionistaClothes, 2) -- koniec klesta -- to daje przepustke do skorowania ludzi
				-- Q5.1
		...
		if player:getStorageValue(Storage.SilenceOfTheLambs.FashionistaClothes) == 2 and player:getStorageValue(Storage.SilenceOfTheLambs.RubMeatWithLecter) < 1 and npcHandler:getTopic(playerId) < 1 then
