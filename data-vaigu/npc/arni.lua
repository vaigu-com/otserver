...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "yes", "tak", "naszyjnik", "necklace" }, message) then
		if player:getStorageValue(Storage.ToCarryThePigs.BeLikeSchwarzenegger) < 1 and npcHandler:getTopic(playerId) == 1 and player:getStorageValue(Storage.StickyBeginning.WoodDelivery) == 6 then
			if player:getStorageValue(Storage.ToCarryThePigs.Main) < 1 then
				player:setStorageValue(Storage.ToCarryThePigs.Main, 1) -- ustawienie glownego questa
			end
			player:setStorageValue(Storage.ToCarryThePigs.BeLikeSchwarzenegger, 1)
	...
	elseif table.contains({ "mission", "misja", "pomoc", "help" }, message) then
		if player:getStorageValue(Storage.ToCarryThePigs.BeLikeSchwarzenegger) == 3 then
			if player:getItemCount(7754) > 0 then
				if player:getLevel() >= 25 then
					player:removeItem(7754, 1) -- zabieramy naszyjnik
					player:setStorageValue(Storage.ToCarryThePigs.BeLikeSchwarzenegger, 4) -- konczymy klesta
		...
		elseif player:getStorageValue(Storage.ToCarryThePigs.BeLikeSchwarzenegger) == 1 then
			if player:getItemCount(10196) >= 5 and player:getItemCount(9685) >= 1 and player:getItemCount(10275) >= 5 then
				player:setStorageValue(Storage.ToCarryThePigs.BeLikeSchwarzenegger, 2) -- wykonana misja
		...
		elseif player:getStorageValue(Storage.TheaterOfCheapThrills.Mission01) == 16 and player:getStorageValue(Storage.ToCarryThePigs.BeLikeSchwarzenegger) == 4 then
			player:setStorageValue(Storage.TheaterOfCheapThrills.Mission01, 17)
		...
		elseif player:getStorageValue(Storage.ToCarryThePigs.BeLikeSchwarzenegger) < 1 then
			if player:getStorageValue(Storage.StickyBeginning.WoodDelivery) == 6 then
			...
			elseif player:getStorageValue(Storage.TheaterOfCheapThrills.Mission01) == 16 and message ~= "pomoc" and message ~= "help" then
