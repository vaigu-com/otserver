...
local function creatureSayCallback(npc, creature, type, message)
	...
	}, message) and player:getStorageValue(Storage.OcellatusXD.Mission01) == 4 then
	...
	elseif MsgContains(message, "yes") or MsgContains(message, "tak") then
		if player:getStorageValue(Storage.OcellatusXD.Mission01) < 1 and npcHandler:getTopic(playerId) == 1 then
			player:setStorageValue(Storage.OcellatusXD.Mission01, 1)
		...
		if player:getStorageValue(Storage.OcellatusXD.Mission01) == 3 and npcHandler:getTopic(playerId) == 2 then
			player:setStorageValue(Storage.OcellatusXD.Mission01, 4)
	...
	elseif table.contains({ "mission", "misja", "misje" }, message) then
		if player:getStorageValue(Storage.OcellatusXD.Mission01) < 1 then
		...
		elseif player:getStorageValue(Storage.OcellatusXD.Mission01) == 1 then
		...
		elseif player:getStorageValue(Storage.OcellatusXD.Mission01) == 2 then
			if player:getItemCount(28818) > 0 then
				if player:getLevel() >= 20 then
					player:setStorageValue(Storage.OcellatusXD.Mission01, 3)
		...
		elseif player:getStorageValue(Storage.OcellatusXD.Mission01) == 3 then
			npcHandler:say(getPlayerLanguage(player) == "PL" and {
			...
			npcHandler:setTopic(playerId, 2)
		elseif player:getStorageValue(Storage.OcellatusXD.Mission01) == 4 then
		...
		elseif player:getStorageValue(Storage.OcellatusXD.Mission01) == 5 then
	...
	elseif table.contains({ "zupa", "skrzydelka", "zarcie", "food" }, message) and player:getStorageValue(Storage.OcellatusXD.Mission01) == 4 then
		if player:getItemCount(9081) > 0 and player:getItemCount(9079) > 0 then
			player:setStorageValue(Storage.OcellatusXD.Mission01, 5)
			player:setStorageValue(Storage.Finished.Ocellatus, 1) -- quest done (website)
...
--end of new definition for npc ocellatus