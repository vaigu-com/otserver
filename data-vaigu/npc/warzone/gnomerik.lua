...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "test") then
		if player:getStorageValueByKey(Storage.BigfootBurden.QuestLine) == 2 then
	...
	elseif MsgContains(message, "A") then
		if npcHandler:getTopic(playerId) == 2 then
			...
			player:setStorageValueByKey(Storage.BigfootBurden.Test, player:getStorageValueByKey(Storage.BigfootBurden.Test) + 7)
	...
	elseif MsgContains(message, "result") then
		if npcHandler:getTopic(playerId) == 33 then
			if player:getStorageValueByKey(Storage.BigfootBurden.Test) < 100 then
				npcHandler:say({
					"You have failed the test with " .. player:getStorageValueByKey(Storage.BigfootBurden.Test) .. " of 112 possible points. You probably were just too nervous. ...",
				...
				npcHandler:say("You have passed the test with " .. player:getStorageValueByKey(Storage.BigfootBurden.Test) .. " of 112 possible points. Congratulations. You are ready to proceed with the more physical parts of your examination! Go and talk to Gnomespector about it.", npc, creature)
				player:setStorageValueByKey(Storage.BigfootBurden.QuestLine, 3)
			end
		end
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			...
			player:setStorageValueByKey(Storage.BigfootBurden.QuestLine, 2)
...
--npcHandler:setMessage(MESSAGE_GREET_ENG, 'Hello and welcome in the gnomish {recruitment} office.')
