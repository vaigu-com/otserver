...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "mission", "misja" }, message) then
		if player:getStorageValueByKey(Storage.BigfootBurden.QuestLine) >= 16 and player:getStorageValueByKey(Storage.BigfootBurden.QuestLine) < 17 then
		...
		elseif player:getStorageValueByKey(Storage.BigfootBurden.QuestLine) >= 17 then
	...
	elseif MsgContains(message, "matchmaker") then
		if player:getStorageValueByKey(Storage.BigfootBurden.MatchmakerTimeout) >= os.time() then
		...
		if player:getStorageValueByKey(Storage.BigfootBurden.MissionMatchmaker) < 1 and player:getStorageValueByKey(Storage.BigfootBurden.MatchmakerTimeout) < os.time() and player:getStorageValueByKey(Storage.BigfootBurden.QuestLine) >= 16 then
			npcHandler:say({
			...
			player:setStorageValueByKey(Storage.BigfootBurden.MissionMatchmaker, 1)
			player:setStorageValueByKey(Storage.BigfootBurden.MatchmakerStatus, 0)
			player:AddCustomItem({id = 15802, count = 1})
			npcHandler:setTopic(playerId, 0)
		elseif npcHandler:getTopic(playerId) == 1 or npcHandler:getTopic(playerId) == 2 then
			if player:getStorageValueByKey(Storage.BigfootBurden.MatchmakerStatus) == 1 and player:removeItem(15801, 1) then
				player:setStorageValueByKey(Storage.BigfootBurden.Rank, player:getStorageValueByKey(Storage.BigfootBurden.Rank) + 10)
				player:AddCustomItem({id = 16128, count = 2})
				player:AddCustomItem({id = 15698, count = 1})
				player:setStorageValueByKey(Storage.BigfootBurden.MissionMatchmaker, 0)
				player:setStorageValueByKey(Storage.BigfootBurden.MatchmakerStatus, -1)
				player:setStorageValueByKey(Storage.BigfootBurden.MatchmakerTimeout, os.time() + 72000)
	...
	elseif MsgContains(message, "repair") then
		if player:getStorageValueByKey(Storage.BigfootBurden.TinkerBellTimeout) >= os.time() then
			return npcHandler:say(getPlayerLanguage(player) == "PL" and "Dzis juz wykonales to zadanie." or "Sorry, you will have to wait before you can undertake this mission again.", npc, creature)
		end

		if player:getStorageValueByKey(Storage.BigfootBurden.MissionTinkersBell) < 1 and player:getStorageValueByKey(Storage.BigfootBurden.TinkerBellTimeout) < os.time() and player:getStorageValueByKey(Storage.BigfootBurden.QuestLine) >= 16 then
			...
			player:setStorageValueByKey(Storage.BigfootBurden.MissionTinkersBell, 1)
			player:setStorageValueByKey(Storage.BigfootBurden.GolemCount, 0)
			player:AddCustomItem({id = 15832, count = 1})
			npcHandler:setTopic(playerId, 0)
		elseif npcHandler:getTopic(playerId) == 1 or npcHandler:getTopic(playerId) == 2 then
			if player:getStorageValueByKey(Storage.BigfootBurden.GolemCount) == 4 and player:removeItem(15832, 1) then
				player:setStorageValueByKey(Storage.BigfootBurden.Rank, player:getStorageValueByKey(Storage.BigfootBurden.Rank) + 10)
				player:AddCustomItem({id = 16128, count = 2})
				player:AddCustomItem({id = 15698, count = 1})
				player:setStorageValueByKey(Storage.BigfootBurden.MissionTinkersBell, 0)
				player:setStorageValueByKey(Storage.BigfootBurden.GolemCount, -1)
				player:setStorageValueByKey(Storage.BigfootBurden.TinkerBellTimeout, os.time() + 72000)
				player:addExperience(30000, true) -- 20k expa
	...
	elseif MsgContains(message, "spore") then
		if player:getStorageValueByKey(Storage.BigfootBurden.SporeGatheringTimeout) >= os.time() then
			return npcHandler:say(getPlayerLanguage(player) == "PL" and "Dzis juz wykonales to zadanie." or "Sorry, you will have to wait before you can undertake this mission again.", npc, creature)
		end

		if player:getStorageValueByKey(Storage.BigfootBurden.MissionSporeGathering) < 1 and player:getStorageValueByKey(Storage.BigfootBurden.SporeGatheringTimeout) < os.time() and player:getStorageValueByKey(Storage.BigfootBurden.QuestLine) >= 17 then
			npcHandler:say({
			...
			player:setStorageValueByKey(Storage.BigfootBurden.MissionSporeGathering, 1)
			player:setStorageValueByKey(Storage.BigfootBurden.SporeCount, 0)
			player:AddCustomItem({id = 15817, count = 1})
			npcHandler:setTopic(playerId, 0)
		elseif npcHandler:getTopic(playerId) == 2 then
			if player:getStorageValueByKey(Storage.BigfootBurden.SporeCount) == 4 and player:removeItem(15821, 1) then
				player:setStorageValueByKey(Storage.BigfootBurden.Rank, player:getStorageValueByKey(Storage.BigfootBurden.Rank) + 10)
				player:AddCustomItem({id = 16128, count = 2})
				player:AddCustomItem({id = 15698, count = 1})
				player:setStorageValueByKey(Storage.BigfootBurden.MissionSporeGathering, 0)
				player:setStorageValueByKey(Storage.BigfootBurden.SporeCount, -1)
				player:setStorageValueByKey(Storage.BigfootBurden.SporeGatheringTimeout, os.time() + 72000)
	...
	elseif MsgContains(message, "grindstone") then
		if player:getStorageValueByKey(Storage.BigfootBurden.GrindstoneTimeout) >= os.time() then
			return npcHandler:say(getPlayerLanguage(player) == "PL" and "Dzis juz wykonales to zadanie." or "Sorry, you will have to wait before you can undertake this mission again.", npc, creature)
		end

		if player:getStorageValueByKey(Storage.BigfootBurden.MissionGrindstoneHunt) < 1 and player:getStorageValueByKey(Storage.BigfootBurden.GrindstoneTimeout) < os.time() and player:getStorageValueByKey(Storage.BigfootBurden.QuestLine) >= 17 then
			npcHandler:say({
			...
			player:setStorageValueByKey(Storage.BigfootBurden.MissionGrindstoneHunt, 1)
			player:setStorageValueByKey(Storage.BigfootBurden.GrindstoneStatus, 0)
			npcHandler:setTopic(playerId, 0)
		elseif npcHandler:getTopic(playerId) == 2 then
			if player:getStorageValueByKey(Storage.BigfootBurden.GrindstoneStatus) == 1 and player:removeItem(15826, 1) then
				player:setStorageValueByKey(Storage.BigfootBurden.Rank, player:getStorageValueByKey(Storage.BigfootBurden.Rank) + 10)
				player:AddCustomItem({id = 16128, count = 2})
				player:AddCustomItem({id = 15698, count = 1})
				player:setStorageValueByKey(Storage.BigfootBurden.MissionGrindstoneHunt, 0)
				player:setStorageValueByKey(Storage.BigfootBurden.GrindstoneStatus, -1)
				player:setStorageValueByKey(Storage.BigfootBurden.GrindstoneTimeout, os.time() + 72000)
				player:addExperience(30000, true) -- 20k expa
