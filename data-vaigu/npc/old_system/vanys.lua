...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif npcHandler:getTopic(playerId) == 4 or npcHandler:getTopic(playerId) == 2 then
		if MsgContains(message, "task") then
			if player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.WardStones.Questline) >= 3 and player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.Main.TheSummerCourt) == 1 and not (player:hasOutfit(1146) or player:hasOutfit(1147)) then
			...
			elseif player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.WardStones.Count) >= 8 and player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.Main.TheSummerCourt) == 1 and player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.WardStones.Questline) == 1 then
			...
			elseif player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.WardStones.Questline) < 1 and player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.Main.TheSummerCourt) < 1 and player:getStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.Main.TheWinterCourt) < 1 then
				...
				player:setStorageValueByKey(Storage.Quest.U12_00.TheDreamCourts.Main.TheSummerCourt, 1)
