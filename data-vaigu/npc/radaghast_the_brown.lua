...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "yes", "tak" }, message) then
		if player:getStorageValue(Storage.WayOfTheDruid.DeerSeason) < 1 and npcHandler:getTopic(playerId) == 1 then
			...
			player:setStorageValue(Storage.WayOfTheDruid.DeerSeason, 1)
	...
	if table.contains({ "mission", "misja", "pomoc", "help" }, message) then
		if player:getStorageValue(Storage.WayOfTheDruid.DeerSeason) == 2 then
			if player:getStorageValue(Storage.WayOfTheDruid.CouncilOfDruids) == 4 then
				npcHandler:say(getPlayerLanguage(player) == "PL" and {
				...
				if player:getLevel() >= 18 then
					player:setStorageValue(Storage.WayOfTheDruid.DeerSeason, 3) -- skonczona misja
		...
		elseif player:getStorageValue(Storage.WayOfTheDruid.DeerSeason) < 1 then
	...
	if table.contains({ "druid", "druidzi", "druidow", "druids" }, message) and player:getStorageValue(Storage.WayOfTheDruid.DeerSeason) == 3 then
