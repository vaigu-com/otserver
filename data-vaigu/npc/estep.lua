...
local function creatureSayCallback(npc, creature, type, message)
	...
	if table.contains({ "yes", "tak" }, message) then
		if player:getStorageValue(Storage.WayOfTheDruid.TakenBenek) < 1 and npcHandler:getTopic(playerId) == 2 then
			player:setStorageValue(Storage.WayOfTheDruid.TakenBenek, 1) -- poczatek benka
			if player:getStorageValue(Storage.WayOfTheDruid.CouncilOfDruids) <= 0 then -- poczatek misji druidow
	...
	if table.contains({ "mission", "misja", "pomoc", "help" }, message) then
		if player:getStorageValue(Storage.WayOfTheDruid.TakenBenek) == 2 then
			if player:getStorageValue(Storage.WayOfTheDruid.CouncilOfDruids) == 4 then
				npcHandler:say(getPlayerLanguage(player) == "PL" and {
				...
				player:setStorageValue(Storage.WayOfTheDruid.TakenBenek, 3) -- skonczona misja
	...
	if table.contains({ "benka", "benek" }, message) then
		if npcHandler:getTopic(playerId) == 1 and player:getStorageValue(Storage.WayOfTheDruid.TakenBenek) < 1 then
		...
		elseif player:getStorageValue(Storage.WayOfTheDruid.TakenBenek) == 1 then
		...
		elseif player:getStorageValue(Storage.WayOfTheDruid.TakenBenek) == 2 then
			npcHandler:say(getPlayerLanguage(player) == "PL" and "Zdolales mu {pomoc}?" or "Did you manage to {help} Benek?", npc, creature)
			npcHandler:setTopic(playerId, 0)
		elseif player:getStorageValue(Storage.WayOfTheDruid.TakenBenek) == 3 then
	...
	if table.contains({ "druidow", "druids" }, message) and player:getStorageValue(Storage.WayOfTheDruid.TakenBenek) >= 3 then
