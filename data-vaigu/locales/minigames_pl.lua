return {
	...
	["MINIGAME_IS_STARTING_MIN_MAX_PLAYERS_ARE_REQUIRED"] = function(context)
		return T(
			":minigameName: Event startuje! Wymagane jest przynajmniej :min: na :max: graczy, pozostalo :minutes: minut aby dolaczyc..",
			{ minigameName = context.minigameName, min = context.min, max = context.max }
		)
	end,
}
