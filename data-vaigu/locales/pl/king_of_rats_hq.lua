return {
	["You cannot use this pipe yet."] = "Nie mozesz jeszcze skorzystac z tego zaworu.",
	["Ancient artifacts of the first King of Rats. I'll keep them for the darkest hour. If you've found any artifact, just tell me which one. If it's legit, I'll unlock one of the pipes for you."] = "Pradawne artefakty pierwszego krola szczurow. Zachowam je na czarna godzine. Jezeli znalazles jakis artefakt to powiedz ktory. Jezeli jest legitny, to odblokuje Ci jedna z rur.",
	--Romek dialogs about artifact
	["Book of rat spells. It will surely come in handy."] = "Ksiega szczurzych czarow. Z pewnoscia sie przyda.",
	["It looks like an ordinary shirt, but it provides the bearer with immunity to mail from the Domestic Violence Monitoring Center."] = "Wyglada jak zwykla zonobijka, ale zapewnia noszacemu odpornosc na pisma z Osrodka Monitorowania Przemocy Domowej.",
	["It is said that this item can bend time itself. Metal part can bend itself, so there is only time missing."] = "Popularna pila ytong. Przewaznie tylko symbolizuje uplyw czasu. Lecz, jesli wierzyc legendom, ta potrafi naprawde naginac czas.",
	["Do you think your Wu-Dka sword can defeat me?"] = "Gdyby istnialo wiecej takich, to nastalby pokoj na swiecie. Nikt nie musialby wiecej innych lac.",
	["Now that i see them myself, they remind me of some other slippers... can't really put my finger on it though."] = "Zobaczywszy je na wlasne oczy stwierdzam, iz przypominaja pewne inne kapcie...",
	["A reliable source of good ol' knowledge on justice."] = "Zrodlo wiedzy o pradawnych tajnikach sprawiedliwosci.",
	["Strapping this to your belt will allow you to farm up to 1000 thousands of gold per hour, but only in dead MMORPGs. It isn't very useful, is it?"] = "Jak przywiazesz go do pasa, to bedziesz mogl wyfarmic nawet 1000 tysiecy golda na godzine, ale tylko w martwych grach mmorpg. Niezbyt to ustrojstwo uzyteczne.",
	--Description onLook
	["Librum Vortex. Romek, the King of Rats, might be interested in it."] = "Librum Vortex. Krol szczurow moze byc nim zainteresowany.",
	["Vestments of Wifebeaterous Regent. Romek, the King of Rats, might be interested in it."] = "Zonobijka wladzy. Krol szczurow moze byc nim zainteresowany.",
	["The Ytong of The Fittest. Romek, the King of Rats, might be interested in it."] = "Ytong mocy. Krol szczurow moze byc nim zainteresowany.",
	["The Bottomless Bud Light. Romek, the King of Rats, might be interested in it."] = "Bezdenny harnas. Krol szczurow moze byc nim zainteresowany.",
	["The Slippers of Bribery. Romek, the King of Rats, might be interested in it."] = "Kapcie przekupstwa. Krol szczurow moze byc nimi zainteresowany.",
	["Sophocles' Antigone. Better than just Antigone. The King of Rats might be intersted in it."] = "Antygona Sofoklesa. Lepsza niz po prostu Antygona. Krol szczurow moze byc nia zainteresowany.",
	["The Silcon Riddle. Romek, the King of Rats, might be interested in it."] = "Sylykonrydel. Krol szczurow moze byc nim zainteresowany.",
	["The Hammer of Epiphany. Romek, the King of Rats, might be interested in it."] = "Mlotek oswiecenia. Krol szczurow moze byc nim zainteresowany.",
	["Cultivator of ULTIMATENESS. The King of Rats might be intersted in it."] = "Kultywator OSTATECZNOSCI. Krol szczurow moze byc nim zainteresowany.",
	-- King of Rats Headquarters
	["King of Rats Headquarters"] = "Loza Krola Szczurow",
	["01. to Ryan"] = "01. do Ryana",
	["02. to Sweaty Cyclops"] = "02. do Spoconego Cyklopa",
	["03. to Djinns"] = "03. do Djinnow",
	["04. to North Kongo"] = "04. do polnocnego Kongo",
	["05. to Werebadgers"] = "05. do Borsukolakow",
	["06. to Vampire city"] = "06. do Miasta Wampirow",
	["07. to East Hurgahada"] = "07. do wschodniej Hurghady",
	["08. to Asuras"] = "08. do Asur",
	["You unlocked this teleport"] = "Odblokowales ten teleport.",
	["Current HeadQuarters status"] = "Status lozy",
	["King of Rats Headquarters Status"] = function(context)
		local unlockedCount = 0
		local collectibleCount = 0
		for _, storage in pairs(Storage.KingOfRatsHQ.CollectibleArtifacts) do
			local portalState = context.player:getStorageValueByKey(storage)
			if portalState == MISSION_FINISHED then
				unlockedCount = unlockedCount + 1
			end
			collectibleCount = collectibleCount + 1
		end

		if unlockedCount == collectibleCount then
			context.player:setStorageValueByKey(Storage.KingOfRatsHQ.Mission01, MISSION_FINISHED)
		else
			context.player:setStorageValueByKey(Storage.KingOfRatsHQ.Mission01, MISSION_STARTED)
		end

		return T("Udalo ci sie zebrac :unlockedCount: z :collectibleCount: artefaktow.", { unlockedCount = unlockedCount, collectibleCount = collectibleCount })
	end,
}
