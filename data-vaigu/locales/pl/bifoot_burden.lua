return {
	...
	["Current reputation points"] = function(context)
		return string.format("Posiadasz %d punktow reputacji.", math.max(context.player:getStorageValueByKey(921), 0))
	end,
	["Daily Minor: Crystal Keeper"] = "Dzienne: Naprawca Krysztalow",
	["Daily Minor: Crystal Keeper Status"] = function(context)
		return string.format("Uzyj krysztalu naprawczego, aby naprawic piec uszkodzonych niebieskich krysztalow w jaskiniach krysztalowych. Uszkodzone krysztaly nie beda swiecic. %d/5", math.max(context.player:getStorageValueByKey(932), 0))
	end,
	...
	["Daily Minor: Raiders of the lost Spark Status"] = function(context)
		return string.format("Zabij Crystal Crushery i uzyj urzadzenia rozladowujacego na ich cialach, aby zebrac ich ladunki. Zbierz 7 ladunkow i zglos sie z nimi. %d/7", math.max(context.player:getStorageValueByKey(934), 0))
	end,
	["Daily Minor Plus: Exterminators"] = "Dzienne Plus: Eksterminacje",
	["Daily Minor Plus: Exterminators Status"] = function(context)
		return string.format("Zabij 10 Wigglerow na pomoc gnomom. Nastepnie zglos sie z powrotem. %d/10", math.max(context.player:getStorageValueByKey(937), 0))
	end,
	["Daily Minor Plus: Mushroom Digger"] = "Dzienne Plus: Niuchacz Grzybow",
	["Daily Minor Plus: Mushroom Digger Status"] = function(context)
		return string.format("Znajdz i zwab swinie truflowa. Czasem wywachuja swieze trufle. Uzyj mlodego prosiaka na truflach, nakarm go 3 razy. Nastepnie zglos sie do gnomow. %d/3", math.max(context.player:getStorageValueByKey(940), 0))
	end,
	...
	["Daily Major: The Tinker's Bell Status"] = function(context)
		return string.format("Uzyj harmonicznego dzwonka na oszlalych golemach w warsztacie golemow, dzieki czemu zostana przetransportowani do warsztatow gnomow. Nastepnie zglos sie z powrotem. %d/4.", math.max(context.player:getStorageValueByKey(946), 0))
	end,
}
