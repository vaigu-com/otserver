...
function baking.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif item.itemid == 6276 and target.itemid == 3464 then -- baking tray
		item:remove(1)
		target:transform(8020, 0) -- baking tray with cookies
	...
	elseif table.contains(oven, target.itemid) then
		...
		elseif item.itemid == 8198 then -- baking tray with garlic cookie dough on it
			...
			item:transform(3464)
		elseif item.itemid == 8020 then -- baking tray cookie dough on it
...
baking:id(3603, 3604, 3605, 6276, 8018, 8020, 8195, 8196, 8198)
baking:register()
