...
function onUseShovel(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif target.itemid == 231 then
        ...
        if target.uid == 11061 and player:getStorageValue(Storage.SciezkaDruida.NaglaEksmisja) == 1 and player:getStorageValue(Storage.SciezkaDruida.SandLock) < 1 then
            toPosition:sendMagicEffect(CONST_ME_FIREAREA)
            player:setStorageValue(Storage.SciezkaDruida.SandLock,1) -- lock piasku
            player:setStorageValue(Storage.SciezkaDruida.NaglaEksmisja,2) -- wykopanie i spalenie kartki
            doCreatureSay(player, 'Ciekawy czar, nawet sam sie spalil po przeczytaniu.', TALKTYPE_ORANGE_1)
        elseif(target.actionid  == 100 and rand <= 20) then
        ...
        toPosition:sendMagicEffect(CONST_ME_POFF)
		elseif target.itemid == 134 and player:getStorageValue(Storage.Yalahar.WstepDoCzarnejMagii) == 2 and player:getStorageValue(target:getUniqueId()) < 1 then
			if target:getUniqueId() > 11048 and target:getUniqueId() < 11059 then -- 11059 = Storage.Yalahar.Groby
				player:setStorageValue(target:getUniqueId(), 1)
				doCreatureSay(player, '*aaaaaaaaaoooooouoouo* - to chyba dzwiek uciekajacych dusz..', TALKTYPE_ORANGE_1)
				local random = math.random()
				print(random)
				if random >= (player:getStorageValue(Storage.Yalahar.Groby) * 0.1) then
					doCreatureSay(player,'Udalo ci sie zdobyc dusze dziewicy!',TALKTYPE_ORANGE_1)
					player:setStorageValue(Storage.Yalahar.WstepDoCzarnejMagii,3) -- udal sie quest, wracamy do Konmulda
				else
					doCreatureSay(player,'Nie udalo sie zlapac duszy!',TALKTYPE_ORANGE_1)
					player:setStorageValue(Storage.Yalahar.Groby,player:getStorageValue(Storage.Yalahar.Groby) - 1) -- dodajemy 10% do szansy przy nastepnym grobie
				end
				...
				doCreatureSay(player,'W tym nie ma sladu zadnych dusz..',TALKTYPE_ORANGE_1)
			end
		else
        return false
    end
...
function onUseMachete(player, item, fromPosition, target, toPosition, isHotkey)
	if table.contains(JUNGLE_GRASS, target.itemid) then
		...
		target:decay()
		return true
	elseif table.contains(VENOREAN_GRASS, target.itemid) then
		target:transform(17151)
		target:decay()
		return true
	elseif table.contains(WILD_GROWTH, target.itemid) then
		toPosition:sendMagicEffect(CONST_ME_POFF)
		target:remove()
		return true
...
function onUseCrowbar(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif target.actionid == 7924 then
		if player:getStorageValue(Storage.TheDreamCourts.TableDoor) < 1 then
			player:addItem(29992, 1)
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'You have found an old lock.')
			player:setStorageValue(Storage.TheDreamCourts.TableDoor, 1)
		else
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'You have already found an old lock.')
		end
	else
		return false
	end
