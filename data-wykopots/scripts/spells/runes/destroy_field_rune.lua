...
function rune.onCastSpell(creature, variant, isHotkey)
	...
	if creature:getStorageValue(Storage.destroLock) == 1 then
		creature:sendCancelMessage("Destroy field is disabled on this event.")
		return false
	end

	if field and table.contains(fields, field:getId()) then
		if field:getActionId() == 25004 or field:getActionId() == 25003 then
