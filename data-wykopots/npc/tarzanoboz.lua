...
local npcType = Game.createNpcType("TarzanOboz")
local npcConfig = {}
...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif table.contains({
		...
		if player:removeMoney(200) then
			local position = Position(7038, 838, 7)
...
npcHandler:setMessage(MESSAGE_GREET, "Elo, z powtorem do {miasta}?")
npcHandler:setMessage(MESSAGE_FAREWELL, "Milego dnia!")
npcHandler:setMessage(MESSAGE_WALKAWAY, "Zegnaj.")
-- npcHandler:setMessage(MESSAGE_GREET_ENG, 'Hello, you want to {sail} back to the city?')
